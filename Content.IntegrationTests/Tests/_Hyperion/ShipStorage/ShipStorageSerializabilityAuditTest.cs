...
namespace Content.IntegrationTests.Tests._Hyperion.ShipStorage
{
    ...
    /// set of state ship storage must capture/strip/rehydrate itself. Static and exhaustive:
    /// no ship-by-ship discovery, no waiting for a populated grid to trip it. The empirical
    /// roster sweep (ShipStorageRosterSweepTest) is the cross-check on this predictor.
    ///
    ...
    /// </summary>
    [TestFixture]
    public sealed class ShipStorageSerializabilityAuditTest
    {
        [Test]
        ...
        private static bool IsConfirmedUnwritable(ISerializationManager serialization, Type type)
        {
            ...
            {
                return true; // the exact gap we care about.
            }
            catch
            {
                return false; // any other failure is a bad sample, not a serializability gap.
            }
        }
